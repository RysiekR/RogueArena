public static class Menu
{
    ...
    public static void GetMenu(Player player)
    {
        ...
        while (continueMenu)
        {
            ...
            Console.WriteLine("Press 3 to show your Inventories");
            //Console.WriteLine("");
            switch (GetInput())
            {
                ...
                case ConsoleKey.D3: OpenInventoryManagment(player); break;
            }
        }
    ...
    private static void OpenInventoryManagment(Player player)
    {
        bool thisMenuContinue = true;
        while (thisMenuContinue)
        {
            Console.Clear();
            Console.WriteLine("This is Inventories Menu(Esc to get back)");
            Console.WriteLine("");

            foreach (InventoryItem inv in player.inventories)
            {
                if (inv != null)
                {
                    inv.DebugInv();
